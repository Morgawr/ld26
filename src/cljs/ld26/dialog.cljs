...
(defn printed-full-message? [screen]
...
(defn update [screen elapsed-time]
  (let [adv (:advance screen)
    ...
    (cond
      (and adv (empty? (rest msgs))) (end-message screen)
      adv (assoc screen 
                 :interval 0 
                 :messages (rest msgs) 
                 :advance false
                 :letters 0)
      (pos? newinterval) (assoc screen 
                                :interval newinterval 
                                :letters (next-letter screen))
      :else (assoc screen :interval interval)
    )
  )
)
...
(defn draw [screen]
  (let [bg (:background screen)
        ...
        msg (first (:messages screen))
        letters (:letters screen)]
    (gimage/draw-image ctx [0 400] bg)
    (gimage/draw-multiline-center-text ctx
                                       [400 450]
                                       (subs msg 0 letters)
                                       "25px"
                                       "white"
