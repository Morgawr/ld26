...
(defn draw-image [ctx pos name]
  (let [image (name @*image-map*)]
      (.drawImage ctx image (first pos) (second pos))
  )
)

(defn fill-clear [canvas ctx color] (set! (. ctx -fillStyle) color)
  (.fillRect ctx 0 0 (.-width canvas) (.-height canvas))
)

(defn draw-text [ctx pos text attr color]
  (set! (. ctx -textBaseline) "top")
  (set! (. ctx -font) (str attr " " *font*))
  (set! (. ctx -fillStyle) color)
  (.fillText ctx text (first pos) (second pos))
)

(defn draw-text-centered [ctx pos text attr color]
  (set! (. ctx -font) (str attr " " *font*))
...
(defn draw-multiline-center-text [ctx pos msg attr color maxlength spacing]
