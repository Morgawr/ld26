...
(def *base-screen* (Screen. "" nil nil nil nil [] nil nil nil nil nil))
...
(defn screen-loop [screen on-top elapsed-time]
  (let [update (:update screen)
        render (:render screen)
        input (:handle-input screen)]
    (-> screen
      (input on-top @ld26.input/*mouse-state*)
...
(defn iterate-screens [screen-list elapsed-time]
...
(defn remove-next-step [screen-list]
  (if (empty? screen-list)
    []
    (doall 
      (map 
        (fn [s]
                  (assoc s :next-frame nil)
        )
        screen-list
      )
    )
...
(defn push-screen [screen screen-list]
  (-> screen-list
    (remove-next-step)
    ((fn [n] (conj (vec n) screen)))
  )
)

(defn pop-screen [screen-list]
...
(defn replace-screen [screen screen-list]
  (->> screen-list
    (pop-screen)
    (push-screen screen)
  )
...
(defn restart-screens [screen screen-list]
  (loop [s screen-list]
    (if 
      (empty? s) [screen]
      (recur (pop-screen s))
...
(defn schedule-next-frame [state]
...
(defn update-time [state curr-time]
  (assoc state :curr-time curr-time)
)

(defn clear-screen [state]
  (ld26.image/fill-clear (:canvas state) (:context state) "black")
  state
)
